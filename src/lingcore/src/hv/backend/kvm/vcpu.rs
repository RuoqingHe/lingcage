...
use kvm_bindings::{KVM_EXIT_RISCV_CSR, KVM_EXIT_RISCV_SBI};
...
use crate::hv::backend::kvm::aarch64;
...
impl Vcpu for KvmVcpu {
    ...
    fn run(&mut self, entry: VmEntry) -> Result<VmExit> {
        ...
        let outcome = loop {
            ...
            match &result {
                ...
                // SBI call or CSR access left to userspace by KVM is answered
                // in the run page and the guest is re-entered.
                #[cfg(target_arch = "riscv64")]
                Ok(VcpuExit::Unsupported(KVM_EXIT_RISCV_SBI | KVM_EXIT_RISCV_CSR)) => {}
                _ => break result,
            }
            #[cfg(target_arch = "riscv64")]
            riscv64::vcpu::answer(self.fd.get_kvm_run());
        };
    ...
    #[cfg(target_arch = "aarch64")]
    fn get_reg(&self, reg: Reg) -> Result<u64> {
        aarch64::vcpu::core_reg(&self.fd, reg)
    }
    ...
    #[cfg(target_arch = "aarch64")]
    fn get_state(&self) -> Result<StateBlob> {
        aarch64::state::VcpuState::capture(&self.fd)
    }

    #[cfg(target_arch = "aarch64")]
    fn set_state(&mut self, blob: &StateBlob) -> Result<()> {
        aarch64::state::VcpuState::restore(&self.fd, blob)
    }
